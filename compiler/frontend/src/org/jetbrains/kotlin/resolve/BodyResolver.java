...
public class BodyResolver {
    ...
    private void resolveConstructorCallForEnumEntryWithoutInitializer(
        ...
        ClassConstructorDescriptor noArgsConstructor = DescriptorUtilsKt.getNoArgsConstructor(enumClassDescriptor);
        if (noArgsConstructor != null && noArgsConstructor.isPrimary()) {
            // Usually enum class has a primary constructor without arguments which is used to initialize all its members:
            //      enum class Color { RED, GREEN, BLUE }
            // Bypass call resolution for this case.
            return;
        }
        ...
        OverloadResolutionResults<FunctionDescriptor> results = callResolver.resolveFunctionCall(
                trace, scopeForConstructor, call, NO_EXPECTED_TYPE, outerDataFlowInfo, false);
